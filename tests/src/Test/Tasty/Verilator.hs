...
instance IsOption Verilator where
  defaultValue = Verilator False
  parseValue = fmap Verilator . safeReadBool
  optionName = pure "verilator"
  optionHelp = pure "Use verilator for tests"
  optionCLParser = flagCLParser Nothing (Verilator True)
...
instance IsTest VerilatorMakeTest where
  run optionSet (VerilatorMakeTest getDir top) progressCallback
    ...
    | otherwise =
        pure (testPassed "Ignoring test: Use --verilator to run")
   where
    mkArgs libs srcs =
      ["-I" <> lib | lib <- libs]
