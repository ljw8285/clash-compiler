...
instance IsOption ModelSim where
  defaultValue = ModelSim False
  parseValue = fmap ModelSim . safeReadBool
  optionName = pure "modelsim"
  optionHelp = pure "Use modelsim for tests"
  optionCLParser = flagCLParser Nothing (ModelSim True)
...
instance IsTest ModelsimVlibTest where
  run optionSet ModelsimVlibTest{mvtSourceDirectory} progressCallback
    ...
    | otherwise =
        pure (testPassed "Ignoring test: Use --modelsim to run")
   where
