...
instance IsOption Symbiyosys where
  defaultValue = Symbiyosys False
  parseValue = fmap Symbiyosys . safeReadBool
  optionName = pure "symbiyosys"
  optionHelp = pure "Use symbiyosys for tests"
  optionCLParser = flagCLParser Nothing (Symbiyosys True)
...
instance IsTest SbyVerificationTest where
  run optionSet SbyVerificationTest {..} progressCallback
    ...
    | otherwise =
        pure (testPassed "Ignoring test: Use --symbiyosys to run")
   where
