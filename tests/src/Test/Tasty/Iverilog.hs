...
instance IsOption Iverilog where
  defaultValue = Iverilog False
  parseValue = fmap Iverilog . safeReadBool
  optionName = pure "iverilog"
  optionHelp = pure "Use iverilog for tests"
  optionCLParser = flagCLParser Nothing (Iverilog True)
...
instance IsTest IVerilogMakeTest where
  run optionSet IVerilogMakeTest{ivmSourceDirectory,ivmTop} progressCallback
    ...
    | otherwise =
        pure (testPassed "Ignoring test: Use --iverilog to run")
   where
